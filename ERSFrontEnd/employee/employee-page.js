window.addEventListener("load", (event) => {
  ...
  h1.innerText = `Welcome to the Employee Portal: ${localStorage.getItem(
    "username"
  )}`;
...
rembursementSubmitBtn.addEventListener("click", async () => {
  ...
  let reimbursementDescriptionInput =
...
async function populateReimbursementTable() {
  ...
  if (res.status === 200) {
    ...
    for (let reimbursemnt of reimbursemnts) {
      ...
      td4.innerText = reimbursemnt.managerUsername
        ? reimbursemnt.dateResolved
        : "Not resolved";
      ...
      td7.innerText = reimbursemnt.managerUsername
        ? reimbursemnt.managerUsername
        : "Not assigned";
